...
def canonize_benchmark(initial: TTreeAut, options: TestOptions):
    ...
    # print(var_order)
    normalized = tree_aut_normalize(unfolded_extra, var_order, verbose=options.debug, output=normalization_log)
...
def simulate_benchmark(input_path: str, vars: int, output_path, simulate=False, images=False):
    ...
    results["04-normal"] = tree_aut_normalize(
        results["03-unfold-extra"], create_var_order("", vars + 1), verbose=True, output=log1
    )
    ...
    results["06-fold"] = tree_aut_folding(
