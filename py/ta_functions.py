...
def tree_aut_determinization(ta: TTreeAut,
                           alphabet: dict,
    ...
    if verbose:
        print("{:<60} {:<20} {:<60} {:<5} {:<5}".format(
            "current_state", "symbol", "children", "work", "done"
        ))
        print("-" * 160)
        counter = 0
    while work_set != []:
...
def tree_aut_intersection(ta1: TTreeAut,
                        ta2: TTreeAut,
                        verbose=False) -> TTreeAut:
    ...
    def handle_intersection_edge(k1: str, e1: list, k2: str, e2: list,
            ...
            for k1, e1 in edges1.items():  # key1, edge1
                for k2, e2 in edges2.items():  # key2, edge2
                    ...
                    if e1.info.label != e2.info.label:  # symbol consistency
                        continue
                    counter += 1
                    handle_intersection_edge(k1, e1, k2, e2,
                                           result, new_state)
...
def tree_aut_product(ta1: TTreeAut, ta2: TTreeAut) -> TTreeAut:
    ...
    def create_product_relation(edge_list: list, alphabet: dict) -> dict:
        ...
        for edge in edge_list:
            ...
            children = [make_name_from_list(edge[2][i])
...
def non_empty_top_down(ta: TTreeAut, verbose=False) -> Tuple[TTreeNode, str]:
    ...
    def output_search_top_down(ta: TTreeAut, state: str, visited: list,
                       out_dict: dict, edge_lookup: dict, debug, total) -> bool:
        if (
        ...
        for edge in ta.transitions[state].values():
            ...
            for child in edge.children:
                ...
                output_reachable = output_search_top_down(ta, child, visited, out_dict,
                                                 edge_lookup, debug, total)
...
def shrink_to_top_down_reachable(ta: TTreeAut) -> TTreeAut:
    ...
    unreachable_states = set(
        i for i in work_ta.get_states() if i not in top_down_reachable_states
    )
...
def is_well_defined(ta: TTreeAut, display_errors=False) -> bool:
    ...
    if (
        set(reachable_bottom_up(ta)) != set(all_states)
        or set(reachable_top_down(ta)) != set(all_states)
    ):
