...
#  Lookup goes like this: symbol -> children -> then the parent/source state is found
...
#  For this it uses a lookup dictionary created at the start of determinization.
...
## This function creates a transition dictionary for the resulting tree automaton,
...
def nonEmptyTD(ta:TTreeAut, verbose=False) -> Tuple[TTreeNode, str]:
    ...
    def outputSearchTD(ta:TTreeAut, state:str, visited:list, outDict:dict, edgeLookup:dict, debug, total) -> bool:
        if ( 
        ...
        for edge in ta.transitions[state].values():
            ...
            for child in edge[2]:
                ...
                outputReachable = outputSearchTD(ta, child, visited, outDict, edgeLookup, debug, total)
            ...
            if outputReachable:
                if debug:
                    ...
                    print("{:<40} {:<5} {:<30}".format(
                        f"{state}"[:40], 
                        f"TRUE", 
                        f"recursion | {visited[l-1]}" if l > 0 else "root"[:30]
                    ))
                outDict[state] = True
